...
function AuthGate() {
  ...
  async function submit(e) {
    ...
    if (mode === 'signup') {
      const { data, error } = await supabase.auth.signUp({ email: email.trim(), password, options: { data: { full_name: fullName.trim(), phone: phone.trim() } } })
      if (error) setMessage(error.message)
      else if (data.session) setMessage('Account created successfully. Welcome to Growvia!')
      else { setMessage('Account created. Please check your email to confirm your account, then sign in.'); setMode('signin'); setPassword('') }
    } else {
