...
export default function AdminWithdrawalActions({ withdrawal, onDone, onMessage }) {
 ...
 async function approve(){if(!confirm(`Approve ₦${Number(withdrawal.amount||0).toLocaleString('en-NG')} for ${withdrawal.account_name||'this user'}?`))return;await run('approve_withdrawal',{p_withdrawal_id:withdrawal.id},'Withdrawal approved.')}
