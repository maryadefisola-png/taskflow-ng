...
export default function HomeUpgrade({ user, profile }) {
  const loc=useLocation(), nav=useNavigate(), [rows,setRows]=useState([]), [tick,setTick]=useState(0)
  useEffect(()=>{
  ...
  return createPortal(<>
    <section className="gv-pro-actions">
      <button onClick={()=>nav("/wallet")}><span>₦</span><b>Deposit</b><small>Add money securely</small></button>
      <button onClick={()=>nav("/wallet")}><span>↑</span><b>Withdraw</b><small>Cash out earnings</small></button>
      <button onClick={()=>nav("/tasks")}><span>✓</span><b>Earn</b><small>Complete tasks</small></button>
    </section>
    <section className="gv-card gv-transactions">
      <div className="gv-section-head"><div><div className="eyebrow">WALLET ACTIVITY</div><h2>Recent transactions</h2></div><button className="gv-text-btn" onClick={()=>nav("/wallet")}>View wallet →</button></div>
